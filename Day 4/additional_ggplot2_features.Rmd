...
## Visualizing Data Distributions
Here we first discuss properties of a variety of distributions and how to visualize distributions using a motivating example of student heights. We then discuss the `ggplot1` geometries for these visualizations in Section \@ref(other-geometries).
...
With the right argument, `ggplot` automatically shades the intersecting region with a different color. We will show examples of `ggplot1` code for densities in Section \@ref(gapminder) as well as Section \@ref(other-geometries).
